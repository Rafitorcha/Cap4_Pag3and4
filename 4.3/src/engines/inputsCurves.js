...
const handleInputOptionsKeyboard = (input) => {
    ...
    if(!Boolean(input.dataset.editable)) input.removeAttribute('readonly')
...
function icValidation({ inputsToValidate }, board, artifact, clone, previousValues) {
    ...
    const response = inputsToValidate.map((objectDefinition, index) => {
        let response = false
    ...
    const validation = response.every((x) => x === true)
    const optionAnimation = info ? info : validation

    if(!info){
        cleanData(artifact, allCurrentResponse, response)
    }
...
function cleanData({ conditions, artifactNumber }, objectResponse, response = null ) {
    const data = {}
